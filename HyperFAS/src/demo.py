...
model = load_model(os.path.join(dirname, "HyperFAS/model/fas.h5"))
...
def test_one(X):
    # moved resizing and scaling to the video reading to speed up the process here and load entire array of frames at once
    # TEMP = X.copy()
    # X = (cv2.resize(X, (224, 224)) - 127.5) / 127.5
    # t = model.predict(np.array([X]))[0]

    # using model directly without .predict() to avoid overhead and speed up prediction generation
